...
When a missing grave is restored, the placement task restores only what is missing: if any of the grave's own hologram lines still stand within the 2-block sweep radius (matched by the `graveHologramGraveUUID:<uuid>` tag / `GRAVE_UUID` key, so a neighbouring grave cannot mask it), only the block is recreated via `BlockManager.createBlock`; otherwise `placeGrave` runs in full. Removing survivors and then placing was rejected: hologram removal and hologram spawning go through two unordered scheduler queues (entity-scheduler removal sweeps vs. region-scheduler spawns), so the sweep could delete the freshly spawned lines (implementation audit).
        ...
        for (Grave grave : new ArrayList<>(graveMap.values())) {
            ...
            checks.add(isGravePlacedAsync(grave).thenAccept(placed -> {
                ...
                plugin.getSchedulerManager().execute(loc, () -> {
                    ...
                    try {
                        if (hasSurvivingHolograms(loc, id)) plugin.getBlockManager().createBlock(loc, grave); else
